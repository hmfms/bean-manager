...
public class JdbcModelReader
{
    ...
    protected void determineAutoIncrementFromResultSetMetaData(Table table, Column[] columnsToCheck) throws SQLException
    {
        ...
        try
        {
        	...
        	final String msg = String.format("impossible determine AutoIncrement From ResultSetMetaData for table [%s]\ncommand=[%s]\n", table.getName(), query.toString());	
        	_log.warn( msg, ex );

        	for( Column c : table.getColumns() ) {
        		_log.trace( String.format( "[%s].[%s]\n", table.getName(), c.getName()) );
        	}
        }
        finally
        {
