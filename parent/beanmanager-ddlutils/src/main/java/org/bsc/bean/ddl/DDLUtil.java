...
public class DDLUtil {
	 ...
	 */
	 public static Table fromBeanInfoToTable( BeanInfo info ) {
	 	 ...
	 	 * @return
	 	 */
		public static String generateSQL(Database db, String Driver, String connectionUrl) {
		    
		    Platform platform = PlatformFactory.createNewPlatformInstance(Driver.trim(), connectionUrl);
		
		    return platform.getCreateTablesSql(db, true /* drop table first*/, true /* continue on error */);

		    //
		    // ADD INITIALIZATION DATA TO SQL
		    //
		    /*
		    SqlDynaBean bean = new SqlDynaBean( SqlDynaClass.newInstance( db.findTable("AUDIT_FILTERS")));
		    	
		    	for( ConfigObject o : getInitData() ) {
		    	
		    		BeanUtils.copyProperties(bean, o);
		        
		    		bean.set( "selected", (o.isSelected()) ? 'Y' : 'N' );
		    		bean.set( "processed", (o.isProcessed()) ? 'Y' : 'N' );
		    		System.out.println(
